...
function GetArticoliPromotion(myRequest) {
        ...
        try {
            ...
            soap.createClient(config.SapWebServiceURL, function (err, client) {
                if (err) {
                    ...
                    client.GetArticoliPromotion({ myLanguageContext, myOffsetRows, myNextRows }, function (err, result) {
                        if (err) {
                            reject(JSON.stringify(
                                new exception(sender, err.message, err.name, err.stack))
                            );
                            ...
                            var resultStatus = response.GetArticoliPromotionResult.Status;
                            var resultError = response.GetArticoliPromotionResult.Error;

                            if (resultStatus == "OK") {

                                /* Inizializza i dati ricevuti dal Web Service */
                                var resultData = response.GetArticoliPromotionResult.Result.diffgram.Data.Articolo;
