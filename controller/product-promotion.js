            ...
            $.ajax({
                ...
                if (response.status == "ERR") {
                    ...
                    $.each(response.data, function (key, product) {
                        ...
                        var partialBadge = '<h6 class="rounded" style="position:relative; width:52px !important; height:20px !important; padding:4px; padding-left:6px; margin-right:-32px; float:right; top:-26px; color:#fff;';
                        if (product.PercentualeSconto != '0.00') {
                            partialBadge += 'background-color:orange;"> - ' + parseFloat(product.PercentualeSconto) + '%';
                        } else {
                            partialBadge += 'background-color:#fff;">';
                        }
                        partialBadge += '</h6>';
                        ...
                        if (product.PercentualeSconto != '0.00') {
                            partialPrezzo = '' +
                                '<h3 style="color:orange;">&euro;&nbsp;' + product.PrezzoUnitarioScontato + '</h3>' +
                                '<h4>&euro;&nbsp; <span style="text-decoration: line-through;">' + product.PrezzoUnitario + '</span></h4>';
                        } else {
                            partialPrezzo = '<h3 style="color:orange;">&euro;&nbsp;' + product.PrezzoUnitario + '</h3><h4>&nbsp;</h4>';
                        }

                        var partialProduct = '' +
                        '<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">' +
                            '<div class="jumbotron" style="padding:20px; border:2px solid orange; background-color:#fff;">' +
                                '<div class="container-fluid">' +
                                    partialBadge +
                                    '<h3 class="glyphicon glyphicon-bookmark" style="position:absolute; left:30px; top:-25px; color:orange;"></h3>' +
